...
export const HeaderSearch = () => {
  ...
  const [categoriesBtnActive, setCategoriesBtnActive] = useState(false);

  const handleCategorySearch = (el: HTMLDivElement, category: string, title: string) => {
    const allItems = Array.from(document.querySelectorAll('.header-search-categories-item')) 
    allItems.map(el => el.classList.remove('active'))
    el.closest('.header-search-categories-item')?.classList.add('active')
  ...
  return (
    <div className={`header-search ${onFocus ? 'focused' : ''}`} aria-label='multi'>
      <div 
        ...
        onClick={() => {
          setCategoriesOpen(state => !state)
          setCategoriesBtnActive(state => !state)
        }}
      >
        <span className="header-search-category-btn-text">All</span>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" className={`header-search-category-btn-icon ${categoriesBtnActive ? 'active' : ''}`} viewBox="0 0 24 24" fill="currentColor" role="presentation"><path fill="none" d="M0 0h24v24H0V0z"></path><path d="M8.71 11.71l2.59 2.59c.39.39 1.02.39 1.41 0l2.59-2.59c.63-.63.18-1.71-.71-1.71H9.41c-.89 0-1.33 1.08-.7 1.71z"></path></svg>
      </div>
      <input 
        type="text" 
      ...
      />
      <div 
        className={`header-search-categories ${categoriesOpen ? 'active' : ''}`}
        onMouseLeave={() => {
      ...
      >
        <div className='header-search-categories-container'>
          <div className='header-search-categories-item active' onClick={(e) => handleCategorySearch(e.target as HTMLDivElement, 'multi', 'All')}>
